    ...
    process {
        ...
        if ($URL -match ".DLL" -or $DLL -eq $true) {
            ...
            try{
                ...
                New-Module -Name "$($ModuleName)" -ScriptBlock ([Scriptblock]::Create($ImportedCode)) | Import-Module
                New-Module -Name "$($ModuleName)" -ScriptBlock ([Scriptblock]::Create((Invoke-WebRequest -Uri $URL -UseBasicParsing -Credential $Credential).Content)) | Import-Module
                #New-Module -Name "$($ModuleName)" -ScriptBlock ([Scriptblock]::Create((Invoke-WebRequest -Uri $URL -UseBasicParsing -Credential $Credential).Content)) -Verbose:($PSCmdlet.MyInvocation.BoundParameters["Verbose"].IsPresent -eq $true) | Import-Module
            }catch{
